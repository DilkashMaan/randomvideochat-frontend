...
const VideoPannel = () => {
    ...
    const cleanupPeerConnection = (preserveLocalVideo = false) => {
        ...
        setHasCalled(false);
        setChatOpen(false);
        setPartnerEmail(null); // ✅ Important
    };
    ...
    const skipPartner = () => {
        console.log("click on skippppp check partner email", partnerEmail);
        ...
        setMessages([]);
        setPartnerEmail(null); // ✅ Clear first
        setIsSearching(true);
        setChatOpen(false);

        setTimeout(() => {
            connectToPartner(); // ✅ Connect AFTER clearing old state
            // setIsSearching(false);
        }, 1000);
    };
